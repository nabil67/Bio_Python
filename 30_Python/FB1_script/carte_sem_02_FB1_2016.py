...
df2 = pd.read_csv("U:/Stagiaires/Nabil.BELAHRACH/Donnees/30_Python/fichiers_csv/02_2016/FB1/Results_20160111-1617.csv",
                  usecols=[1,3,4,7,8,9,10], sep = ";", header = False, 
                  names = ['nom','moyenne','ecart-type','solidite','entropie','smoothness','mm_gradient'])  

df3 = pd.read_csv("U:/Stagiaires/Nabil.BELAHRACH/Donnees/30_Python/fichiers_csv/02_2016/FB1/Results_20160112-0926.csv",
                  usecols=[1,3,4,7,8,9,10], sep = ";", header = False, 
                  names = ['nom','moyenne','ecart-type','solidite','entropie','smoothness','mm_gradient'])                  
                  # bon                  

df4 = pd.read_csv("U:/Stagiaires/Nabil.BELAHRACH/Donnees/30_Python/fichiers_csv/02_2016/FB1/Results_20160112-1142.csv",
                  usecols=[1,3,4,7,8,9,10], sep = ";", header = False, 
                  names = ['nom','moyenne','ecart-type','solidite','entropie','smoothness','mm_gradient'])    
...
nom_figure_moyenne = 'la courbe moyenne de la semaine 02_2016  FB1_logE {:}'.format(time.strftime("%Y-%m-%d-%H:%M"))
nom_figure_entropie = 'la courbe entropie de la semaine 02_2016 FB1_logE {:}'.format(time.strftime("%Y-%m-%d-%H:%M"))
...
if __name__ == "__main__": 
    ...
    date_to_hour(df)   
    data02 = df_semaine(df, file_name, semaine, FB)
    data02 = convert_to_hours(data02) 
    print("Nombre d'essais:", df.num_essai[len(df) - 1])
