...
df3 = pd.read_csv("U:/Stagiaires/Nabil.BELAHRACH/Donnees/30_Python/fichiers_csv/22_2016/FB2-opt/Results.csv",
                  usecols=[1,4,5,8,9,10,11], sep = ";", header = False, 
                  names = ['nom','moyenne','ecart-type','solidite','entropie','smoothness','mm_gradient'])                  
...
df = pd.concat([  df3] , axis=0, ignore_index=True)  
...
if __name__ == "__main__": 
    ...
    date_to_hour(df)   
    data22FB2opt = df_semaine(df, file_name, semaine, FB)
    data22FB2opt = convert_to_hours(data22FB2opt)                                  
